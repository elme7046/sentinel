...
class GUI():
	def __init__(self, root):
		...
		self.screen_panel = tk.Frame(self.root, width=800, height=600)
		...
		#Screen Panel Assets
		self.top_panel = tk.Frame(self.screen_panel, width=800, height=520, background="#B0B0B0")
		self.top_panel.pack(side="top")
		self.top_panel.pack_propagate(False)

		self.prev_button = tk.Button(self.top_panel, text="<", width=3, height=2,
							   command=self.prev_image)
		self.prev_button.pack(side="left")

		self.canvas = Canvas(self.top_panel, width=CANVAS_W, height=CANVAS_H)
		self.canvas.pack(side="left", padx=(1,1))
		
		self.next_button = tk.Button(self.top_panel, text=">", width=3, height=2,
							   command=self.next_image)
		self.next_button.pack(side="right")
		
		self.bottom_panel = tk.Frame(self.screen_panel, width=800, height=60, background="#FF0000")
		...
		self.list_label = tk.Label(self.functions_panel, width=20, height=2, text="No List", font=("Arial 10"))
		self.list_label.pack(side="top", pady=(10,10))

		self.current_face_name = tk.Label(self.functions_panel, width=20, height=2, text="", font=("Arial 10"))
		self.current_face_name.pack(side="top")

		self.face_name = tk.Entry(self.functions_panel, width=20, font=("Arial 10"))
		...
		self.remove_whitelist_button = tk.Button(self.functions_panel, width=20, height=3, text="Refresh Database",
	...
	def get_images(self, reset=False):
		if(reset):
			...
			self.faces_whitelist = []
			self.faces_blacklist = []
		for root, dirs, files in os.walk("/home/senorita/sentinel/python/sentinel_database/", topdown=False):
			for name in files:
				if '.png' in name:
	...
	def next_image(self):
		...
		self.list_label["text"] = self.list_text + " (" + str(self.image_index+1) + "/" + str(len(self.image_list)) + ")"
	...
	def update_canvas(self, image=None):
		...
		if(type(image) == FaceWithData):
			face = image.get_face()
			img = Image.open(face)
			self.img = ImageTk.PhotoImage(img.resize((CANVAS_W,CANVAS_H)))
			self.current_face_name["text"] = image.get_name()
		else:
			pic = Image.open("./default.png")
			self.img = ImageTk.PhotoImage(pic.resize((CANVAS_W,CANVAS_H)))
